...
为使 Python 3.10+ 基础环境无需 CASA/casacore 也能完成教学复盘，`extract_visibility.py` 从原 MS 导出 3C273 校准场和一个 NGC 4826 field/DDID 的 `DATA`、`FLAG`、`UVW`、`WEIGHT`、天线和时间列，并另存全部七个目标 field、四个 64 通道 data description 的行级 `UVW`、元数据和 flag 比例。`analyze_ms.py` 还生成逐积分相对增益表，记录参考天线、输入支持权重、flag、14 条训练基线、7 条留出基线及逐时间残差；`derived/ms_metadata.yaml` 固定保存 SOURCE、SYSCAL、OBSERVATION 和 HISTORY 的证据摘要。输入支持权重不是增益形式协方差。最终增益使用全部 21 条基线重算，留出解只用于验证。所有派生数组均被校验和固定。子表显示 `TSYS`、`BIMA_JYPERK` 和上游校准开关存在，但主表 `DATA` 没有物理单位，SOURCE 没有通量列，因而不能把上游校准痕迹当成独立绝对通量模型。校准场结束到目标场开始相隔约 989 s，因此本案例不声称可把相对增益直接同步转移到目标。该包支持表结构、flag、$uv$ 覆盖、相对标量增益表和基线留出 QA，但因缺少独立通量模型、完整日志和原归档身份，不能建立绝对通量标度或声称完成可发布的重成像。

`9_Practical/archive_labs/vla_3c391/` 记录 NRAO/CASA VLA 3C391 教程数据的外部获取与复现契约。观测身份为项目 `TDEM0001`、执行块 `TDEM0001_sb1218006_1.55310.33439732639`、VLA D 构型、2010-04-24；数据 URL、3,271,507,505 字节长度和 SHA-256 `9152b1ce8603a3b0ffd50ce3d3c57f53a82e6fde89fd6a4b5fa4f8c7d8481910` 固定在 manifest 和下载脚本中。NRAO 在专有期后提供公开访问，但本轮未找到该教程 MS 的独立再分发许可证，因此仓库不包含压缩包、展开 MS、FITS 或官方教程截图，也不把第三方数据改用 GPLv2。

仓库只提交本项目编写的下载、CASA 流水线和结果审计脚本，以及由 2026-07-28 验证运行得到的机器可读参考数值。验证使用 CASA 6.7.0.31、casadata 2025.9.22、Perley-Butler 2017 的 3C286 模型，并从教程起点生成延迟、带通、增益和 fluxscale 表。参考成像的固定大圆 mask 会触发大尺度负分量或发散停止，残差仍有相关结构；这些数值只定义可复现课程基线，不构成官方数据再发布或发表级图像声明。
