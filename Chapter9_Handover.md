...
- 一条可运行且更接近真实工作的谱线处理链：line-free 通道选择、连续谱基线模型比较、channel map、平滑辅助 masking、PV 图与 `W20/W50` 线宽诊断。
...
- `9.7` 已从“最小谱线原型”加厚为更完整的训练页，加入了 line-free 选择对比、线性基线拟合、平滑辅助 mask、PV 图以及 `W20/W50` 轮廓诊断；
...
当前 `9.7` 已经不再只是最小工作流，而是覆盖了 line-free 选择、基线模型、mask、PV 和基础线宽测量的综合练习。但若要达到成熟训练教程的深度，后续仍建议继续补强：

- 更真实的 uv-domain continuum subtraction 场景；
- 更系统的 3D mask 与 source finding 接口；
- 更完整的线型拟合与多分量情形；
- PV 图和旋转曲线、外流或非圆周运动之间的联系；
- 与物理量估计的连接，例如柱密度、总气体质量或旋转趋势的入门解释。
