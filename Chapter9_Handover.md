...
- 一条可运行的基础谱线处理链：line-free 通道选择、continuum subtraction、channel map、moment map 与基本线宽测量。
...
- `9.7` 已加入第一版谱线实践，把 continuum subtraction、channel map、moment map 和积分谱测量串成一条最小工作流；
...
当前 `9.7` 已经建立了第一版谱线最小工作流，但若要达到成熟训练教程的深度，后续仍建议继续补强：

- 更真实的 continuum subtraction 场景；
- cube masking 与噪声偏差控制；
- PV diagram、位置-速度切片；
- 更完整的线型拟合与多分量情形；
...
- 第 9 章现在已经从“零散实践页”升级成“连续谱主线 + 第一版谱线实践”；
- 但它仍只是 P0 路线中的基础层；
